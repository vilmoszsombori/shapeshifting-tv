...
/**
 * This servlet may be used to create bitmap versions of the graphs using a
 * high-level description of the visual appearance so there is no need to create
 * an object representation of the model on the server-side. The description
 * even allows the server to create the bitmaps using a SAX parser. A DOM parser
 * is not required.
 * 
 * To integrate the image handler with a client application, the client
 * application must be setup to use this servlet. This can be done by setting
 * mxEditor.urlImage programmatically or using a config file.
 */
public class Export extends HttpServlet {

	private static final long serialVersionUID = -4920699330412095513L;

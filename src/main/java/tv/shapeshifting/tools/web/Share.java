...
import tv.shapeshifting.tools.mxShapeshifting;
...
/**
 * This servlet demonstrates the sharing of diagrams across a number of clients.
 * The servlet itself does not have an object representation of the graph in
 * memory, it only serves as a dispatcher for the XML among the clients.
 * 
 ...
 * connect the model to the backend. Note that it is possible to attach multiple
 * sessions to one editor and graph model.
 * 
 ...
 * encoded changes to those clients. (Note that the sender will not get notified
 * of his own changes in the default mode.)
 * 
 * When the client receives such a set of changes, it decodes them and executes
 * them on its local model, bypassing the command history of local changes. This
 * means the client immediately sees the changes, but pressing undo will only
 * undo the last local change of that specific client.
 * 
 ...
 * mxEditor node in the config file. For diagram sharing, the following member
 * variables are used: mxEditor.urlInit, mxEditor.urlPoll and
 * mxEditor.urlNotify.
 */
public class Share extends HttpServlet {

	private static final long serialVersionUID = 6984565649954875399L;
	private static Logger LOG = Logger.getLogger(Share.class);
	 ...
	 */
	protected static boolean useSharedModel = true;
	...
	protected static mxSharedState sharedResource = (useSharedModel) ? new mxSharedGraphModel(	
			new mxGraphModel() {
				...
				private static final long serialVersionUID = 8802573762587637670L;
	...
	protected static mxGraph graph = new mxGraph(((mxSharedGraphModel)sharedResource).getModel());
	

	// Associates the graph model codec with the above anonymous class
	static {
		mxCodecRegistry.addAlias("tv.shapeshifting.tools.web.Share$1",
				"mxGraphModel");
		sharedResource.addDiagramChangeListener(new  mxDiagramChangeListener() {
			
			@Override
			public void diagramChanged(Object sender, String edits) {
				LOG.debug("Model changed: " + sender + " [" + edits + "]");
	...
	protected void doPost(HttpServletRequest request,
		...
		LOG.debug(session.getId() + " post xml = " + xml);
	...
	protected void doGet(HttpServletRequest request,
		...
		// Redirects the client to the actual static diagram editor. The diagram
		// editor
		// loads the configuration which contains one hook to request more
		// configuration
		// data at the server. This request is implemented in Config.java to
		// deliver the
		// configuration required to access this servlet for sharing and
		// Export.java for
		// creating images.
		...
		} else if (query.equalsIgnoreCase("dummy")) {
			// Creates the graph on the server-side
			LOG.debug("Session: " + request.getSession(false).getId());
			if (request.getSession(false) != null && request.getSession(false).getAttribute(Settings.ONTOLOGY) != null) {
				OntologyInterface ow = (OntologyInterface)request.getSession(false).getAttribute(Settings.ONTOLOGY);
				mxShapeshifting adapter = new mxShapeshifting(((mxSharedGraphModel)sharedResource).getModel(), ow.getOntModel());
				adapter.convert();
			} else {
